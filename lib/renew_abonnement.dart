...
class _PaymentPageState extends State<MyApp_renew> {
      ...
      if (response.statusCode == 200) {
        ...
        if (status == "ACCEPTED") {
          ...
          if (storedTransaction != null) {
            ...
            final userEmail = transactionData['email'];

            // Génération automatique des valeurs pour les dates
            ...
            final existingSubscriptions = await subscriptionsRef
                .where("emailclit", isEqualTo: userEmail)
                .get();

            if (existingSubscriptions.docs.isNotEmpty) {
              // Mise à jour de l'abonnement existant (seulement creationDate, expirationDate et status)
              final existingSubscriptionId =
                  existingSubscriptions.docs.first.id;
              addLog(
                  "Abonnement existant trouvé, mise à jour: $existingSubscriptionId");

              await subscriptionsRef.doc(existingSubscriptionId).update({
                "creationDate": Timestamp.fromDate(creationDate),
                "expirationDate": Timestamp.fromDate(expirationDate),
                "status": "Actif",
              });

              addLog("🔄 Réabonnement effectué avec succès !");
            } else {
              // Création d'un nouvel abonnement
              addLog(
                  "Aucun abonnement existant, création d'un nouvel abonnement");
              final String uniqueCode = generateUniqueCode();

              await subscriptionsRef.doc(transactionId).set({
                "transaction_id": transactionId,
              ...
              addLog("🔥 Nouvel abonnement enregistré dans Firestore !");
            }
            ...
            if (mounted) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content:
                      Text("Paiement réussi ! Votre abonnement est actif."),
